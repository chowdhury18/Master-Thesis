...
spec = importlib.util.spec_from_file_location("module.name", "/home/arnab/Desktop/DNN/gRPC/Users/resnet34/resnet34.py")
resnet34 = importlib.util.module_from_spec(spec)
spec.loader.exec_module(resnet34)

custom_logging_format = '%(asctime)s : [%(levelname)s] - %(message)s'
logging.basicConfig(filename= "/home/arnab/Desktop/DNN/gRPC/logs/gRPC_time_state_resnet34_cpu_match_part.log" , filemode="a", level= logging.INFO, format=custom_logging_format)
...
def send_message(NN, img_part, part_inx, BATCH_SIZE, current_layer, epoch, pretrained_model, prev_input_units):
    try:
        ...
        img_part = img_part.detach().numpy().tobytes() # Can't call numpy() on Variable that requires grad. Use var.detach().numpy() instead.
        if NN == "conv":
...
def main():
    ...
    try:
        ...
        csv_file = open('/home/arnab/Desktop/DNN/gRPC/logs/gRPC_time_state_resnet34_cpu_match_part.csv','a')
        ...
        for img,level in loader:
            ...
            for i in range(len(cfgs)):
                ...
                for j in range(len(after_part)):
                    ...
                    if j == matched_fog_node_CA_idx:
                        ...
                        s = time.time()
                        img_part = resnet34.nn_layer('conv', img_part, current_layer, model, 0, model_dict)
                        e = time.time()
            ...
            for j in range(worker):
                ...
                if j == matched_fog_node_CA_idx:
                    ...
                    s = time.time()
                    img_part = resnet34.nn_layer('ff', img_part, current_layer, model, 0, model_dict)
                    e = time.time()
                    layer_comp_time = (e-s)*1000
                    m = nn.ReLU()
                    out = m(img_part).data > 0
