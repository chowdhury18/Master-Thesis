...
logging.basicConfig(filename= "/home/arnab/Desktop/DNN/gRPC/logs/gRPC_time_state_alexnet_cpu_match_part.log" , filemode="a", level= logging.INFO, format=custom_logging_format)
...
def received_message(msg, current_layer, fog_node):
    ...
    try:
        ...
        if message[0] == "conv":
            ...
            if fog_node == 0:
                print(f"fog1: current: {current_layer} trans: {(trans_user + trans_fog)} data: {sent_data/(1024*1024)} comp: {layer_comp_time}")
                set_value(current_layer, (trans_user + trans_fog), sent_data/(1024*1024), layer_comp_time)

            elif fog_node == 1:
                print(f"fog2: current: {current_layer} trans: {(trans_user + trans_fog)} data: {sent_data/(1024*1024)} comp: {layer_comp_time}")
                set_value(current_layer, (trans_user + trans_fog), sent_data/(1024*1024), layer_comp_time)
...
def reset(worker):
    ...
    sent_goodput = np.zeros((worker,6)) # worker = 2
    received_goodput = np.zeros((worker,6)) # worker = 2
...
sent_goodput = np.zeros((2,6)) # worker = 2
received_goodput = np.zeros((2,6)) # worker = 2
def main():
